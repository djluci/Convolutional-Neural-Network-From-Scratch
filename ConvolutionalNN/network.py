...
import layer
import filter_ops
import accelerated_layer as al
...
class Network:
    ...
    def forward(self, inputs, y):
        ...
        # get loss from last layer (unregularized)
        last_layer = self.layers[-1]
        loss = last_layer.loss(y)
    ...
    def predict(self, inputs):
        ...
        '''
        # before doing forward pass, set layers to operate in test/prediction/evaluation/inference mode
        original_training_mode = self.is_training() # save our original state
        self.set_training_mode(False) # set network and layers to test mode
        ...
        # softmax output layer gives class scores, take argmax
        pred_classes = np.argmax(out, axis=1)

        # after forward pass, set layers to operate in whatever mode they were at start
        self.set_training_mode(original_training_mode) # restore original state
    ...
    def fit(self, x_train, y_train, x_validate, y_validate, mini_batch_sz=256, n_epochs=1, print_every=10):
        ...
        '''
        original_network_training_state = self.is_training() # save original network state
        self.set_training_mode(True) # set network and its layers to training mode

        num_train = x_train.shape[0]
        total_iters = (num_train // mini_batch_sz) * n_epochs
        iter_count = 0
        start_time = time.time() # initialize start_time here for consistent elapsed calculation
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(0, num_train, mini_batch_sz):

                # ===== Mini-batch =====
                batch_idx = perm[i:i + mini_batch_sz]
                xb = x_train[batch_idx]
                yb = y_train[batch_idx]

                # ===== Forward =====
                loss = self.forward(xb, yb)
                ...
                # ===== Weight updates =====
                for li in self.wt_layer_inds:
                    self.layers[li].update_weights()

                # ===== Print stats =====
                if iter_count % print_every == 0:

                    if iter_count == 0:
                        print("Iteration 0: computing timing estimates...")
                        # start_time is already defined outside loop
                        
                    elapsed = (time.time() - start_time) / 60
                    # for i 0, iter_count is 0, so max ensures no division by zero errors
                    remaining = (total_iters - iter_count) * (elapsed / max(iter_count, 1))
                    ...
                    print(f"[Iter {iter_count}/{total_iters}] "
                        f"Loss: {loss:.4f}, "
                        f"Train Acc: {train_acc:.3f}, Val Acc: {val_acc:.3f}")

                    if iter_count == 0:
                        print(f"Elapsed: {elapsed:.2f}m, Projected total: {remaining:.2f}m")

                iter_count += 1

        # when training session ends: set network and all layers to not be in training mode
        self.set_training_mode(original_network_training_state) # restore original network state
...
class ConvNet4(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
        ...
        # Layer 0: Conv2D
        conv_layer = layer.Conv2D(number=layer_num, name='conv', n_kers=n_kers[0], ker_sz=ker_sz[0],
                            n_chans=n_chans, wt_scale=wt_scale, activation='relu', reg=reg,
                            r_seed=r_seed, verbose=verbose)
        self.layers.append(conv_layer)
        layer_num += 1

        # Layer 1: MaxPool2D
        pool_layer = layer.MaxPool2D(number=layer_num, name='pool', pool_size=pooling_sizes[0],
                               strides=pooling_strides[0], verbose=verbose)
        self.layers.append(pool_layer)
        layer_num += 1

        # Compute output size after Conv2D + MaxPool2D for Flatten layer
        h_pool = (h - pooling_sizes[0]) // pooling_strides[0] + 1
        w_pool = (w - pooling_sizes[0]) // pooling_strides[0] + 1
        n_feats_flat = n_kers[0] * h_pool * w_pool

        # Layer 2: Flatten
        flatten_layer = layer.Flatten(number=layer_num, name='flatten', verbose=verbose)
        self.layers.append(flatten_layer)
        layer_num += 1

        # Layer 3: Dense (hidden)
        dense1 = layer.Dense(number=layer_num, name='dense1', units=dense_interior_units[0],
                       n_units_prev_layer=n_feats_flat, wt_scale=wt_scale, activation='relu',
                       reg=reg, r_seed=r_seed, verbose=verbose)
        self.layers.append(dense1)
        layer_num += 1

        # Layer 4: Dense (output)
        dense2 = layer.Dense(number=layer_num, name='dense_out', units=n_classes,
                       n_units_prev_layer=dense_interior_units[0], wt_scale=wt_scale,
                       activation='softmax', reg=reg, r_seed=r_seed, verbose=verbose)
        self.layers.append(dense2)

        # Define indices of layers with weights
...
class ConvNet4Accel(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
        ...
        # Layer 0: Conv2DAccel
        conv_layer = al.Conv2DAccel(number=layer_num, name='conv', n_kers=n_kers[0], ker_sz=ker_sz[0],
                            n_chans=n_chans, wt_scale=wt_scale, activation='relu', reg=reg,
                            r_seed=r_seed, verbose=verbose)
        self.layers.append(conv_layer)
        layer_num += 1
        ...
        # Layer 1: MaxPool2D
        pool_layer = al.MaxPool2DAccel(number=layer_num, name='pool', pool_size=pooling_sizes[0],
                               strides=pooling_strides[0], verbose=verbose)
        self.layers.append(pool_layer)
        layer_num += 1
...
class ConvNet4AccelV2(Network):
    '''A minimal convolutional neural network with dropout regularization.
    Makes a ConvNet4 network with the following layers: Conv2D -> MaxPool2D -> Flatten -> Dense -> Dropout -> Dense
    ...
    '''
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(7,), dense_interior_units=(100,),
                 pooling_sizes=(2,), pooling_strides=(2,), n_classes=10, wt_scale=1e-2, reg=0,
                 dropout_rate=0.0, r_seed=None, verbose=True):
        ...
        dropout_rate: float. Between 0 and 1.
            The proportion of units to drop in the Dropout layer. 0.0 means no dropout.
        r_seed: int or None.
            Random seed for setting weights and bias parameters.
        verbose: bool. Do we want to term network-related debug print outs on?
            NOTE: This is different than per-layer verbose settings, which are turned manually on below.
        ...
        # Layer 4: Dropout
        dropout_layer = layer.Dropout(number=layer_num, name='dropout', rate=dropout_rate,
                                      r_seed=r_seed, verbose=verbose)
        ...
        # Define indices of layers with weights (Dropout, MaxPool2D, Flatten have none)
