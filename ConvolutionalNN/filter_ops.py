...
def conv2nn(imgs, kers, bias, verbose=True):
    ...
    if verbose:
        print(f'batch_sz={batch_sz}, n_chan={n_chans}, img_x={img_x}, img_y={img_y}')
    ...
    if n_chans != n_ker_chans:
        raise ValueError('Number of kernel channels must match input num channels!')

    # Padding for 'same' convolution
    pad_x = math.ceil((ker_x - 1) / 2)
    pad_y = math.ceil((ker_y - 1) / 2)

    # Pad each image in the batch
    padded_imgs = np.pad(imgs, ((0, 0), (0, 0), (pad_y, pad_y), (pad_x, pad_x)), mode='constant', constant_values=0)

    if verbose:
        print(f'Padded image shape: {padded_imgs.shape}')

    # Output array
    output = np.zeros((batch_sz, n_kers, img_y, img_x))

    # Perform convolution
    for b in range(batch_sz):  # iterate over images in batch
        for k in range(n_kers):  # iterate over filters
            kernel_flip = kers[k, :, ::-1, ::-1]  # flip kernel in x and y
            for y in range(img_y):
                for x in range(img_x):

                    window = padded_imgs[b, :, y:y+ker_y, x:x+ker_x]
                    conv_val = np.sum(window * kernel_flip)
                    output[b, k, y, x] = conv_val + bias[k]
