...
class Dropout(Layer):
    ...
    def compute_net_in(self):
        ...
        if self.in_train_mode:
            # Values below (1-rate) survive (True), above are dropped (False)
            self.mask = (self.rng.random(self.input.shape) < (1 - self.rate))
            #scale surviving units UP by 1/(1-rate) during training so no rescaling is needed at test time
            self.net_in = self.input * self.mask / (1 - self.rate)
        else:
            # Test/predict mode: pass everything through unchanged
    ...
    def backward_netIn_to_prevLayer_netAct(self, d_upstream):
        ...
        going forward.
        '''
        # Gradient only flows back through units that survived the forward pass.
